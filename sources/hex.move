...
module razor_libs::hex {
    ...
    const ERR_INVALID_HEX_LENGTH: u64 = 0;
    const ERR_NOT_VALID_HEX_CHARACTER: u64 = 1;
        ...
        while (i < l) {
            vector::append(
              &mut r, (
                *vector::borrow(
                  &hex_vector, (
                    (*vector::borrow(&bytes, (i as u64))) as u64
                  )
                )
              )
            );
    ...
    public fun decode(hex: vector<u8>): vector<u8> {
        ...
        assert!(l % 2 == 0, ERR_INVALID_HEX_LENGTH);
        while (i < l) {
            let decimal = 
              decode_byte(*vector::borrow(&hex, (i as u64))) * 16 + 
              decode_byte(*vector::borrow(&hex, ((i + 1) as u64)));
    ...
    fun decode_byte(hex: u8): u8 {
        if (/* 0 .. 9 */ 48 <= hex && hex < 58) {
            ...
            abort ERR_NOT_VALID_HEX_CHARACTER
        }
    }
    ...
    #[test]
    #[expected_failure(abort_code = ERR_INVALID_HEX_LENGTH)]
    fun test_hex_decode__invalid_length() {
        decode(b"0");
    }

    #[test]
    #[expected_failure(abort_code = ERR_NOT_VALID_HEX_CHARACTER)]
