module razor_libs::i64 {
    use std::error;

    const ERROR_OVERFLOW: u64 = 0;
    ...
    public fun from(v: u64): I64 {
        assert!(v <= MAX_AS_U64, error::invalid_argument(ERROR_OVERFLOW));
        I64 {
            bits: v
        }
    }
    ...
    public fun neg_from(v: u64): I64 {
        assert!(v <= MIN_AS_U64, error::invalid_argument(ERROR_OVERFLOW));
        if (v == 0) {
            I64 {
                bits: v
            }
    ...
    public fun add(num1: I64, num2: I64): I64 {
        ...
        assert!(overflow == 0, error::invalid_argument(ERROR_OVERFLOW));
        sum
    }
    ...
    public fun abs(v: I64): I64 {
        if (sign(v) == 0) {
            ...
            assert!(v.bits > MIN_AS_U64, error::invalid_argument(ERROR_OVERFLOW));
            I64 {
